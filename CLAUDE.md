...
0. **计划文档归属** — 本项目的所有 Plan 文件存放在 `docs/plans/`，不使用 `~/.claude/plans/`。
...
- context-management: LLM 对话上下文管理：多层架构（system prompt / 结构化记忆槽 / 窗口裁剪 / fallback）、设计决策记录、放弃的方案、未来规划 → docs/tech/context-management.md
- deployment: 生产环境部署：阿里云 ECS（宝塔面板）、GitHub 仓库、Node.js + PM2 + Nginx、更新流程、常见问题 → docs/tech/deployment.md
- react-patterns: React 19 and Next.js 16 specific patterns, Strict Mode pitfalls, async cookies, useRef, useOptimistic, RSC boundaries → docs/tech/react-patterns.md
- technical-architecture: 项目技术架构全貌：技术栈选型、RSC+客户端岛屿架构、三层 Context 状态管理、SSE 流数据管道、路由体系、样式方案、测试策略 → docs/tech/technical-architecture.md
<!-- TECH_REFS_LIST:END -->
